...
CREATE TABLE StripeDonor(
    ...
    PRIMARY KEY(donorID),
    FOREIGN KEY(donorID) REFERENCES AuthenticationInformation(Username)
);
...
CREATE TABLE StripeRecipient(
    ...
    FOREIGN KEY(recipientID) REFERENCES AuthenticationInformation(Username)
);
...
CREATE TABLE StripeHUG(
    ...
    FOREIGN KEY(recipientID) REFERENCES StripeRecipient(recipientID)
);
...
CREATE TABLE Fee(
    ...
    FOREIGN KEY(HUGID) REFERENCES StripeHUG(HUGID)
);
...
CREATE TABLE Card(
    ...
    FOREIGN KEY(donorID) REFERENCES StripeDonor(donorID)
);

CREATE TABLE ConnectedBankAccount(
    bankAccountID INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    stripeBankAccountID VARCHAR(255),
	userID VARCHAR(255),
    bank_nickname VARCHAR(255),
    Verified BOOLEAN
    /*No foreign key here, as userID could be either donorID OR recipientID */
);
...
CREATE TABLE BankAccount(
    ...
    FOREIGN KEY(userID) REFERENCES AuthenticationInformation(id) ON DELETE CASCADE,
    FOREIGN KEY(stripeCustomerID) REFERENCES UserStripe(stripeCustomerID) ON DELETE CASCADE
);
