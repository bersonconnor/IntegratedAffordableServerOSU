...
export class WebhookService {
  ...
  private static instance = new WebhookService();

  public static getInstance = () => WebhookService.instance;


  public transferComplete = async (transfer) => {
    ...
    const updateBalance = 
    `UPDATE StripeRecipient SET Balance=Balance - ? 
      WHERE recipientID = ? 
    `;
  ...
  */
  ...
  public chargeFailed = async (pending) => {
  ...
  public chargePending = async (pending) => {
    const chargeID = pending.data.object.id;
    const amount = parseFloat(pending.data.object.amount) / 100; //default Stripe unit is cents, not dollars
    const customerID = pending.data.object.customer;
  ...
  public chargeSucceeded = async (charge) => {
