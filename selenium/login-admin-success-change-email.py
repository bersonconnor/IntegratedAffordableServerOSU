...
#README: Navigate to Login page as admintest1234, and fail the login successfully, and then change email on the account.

#Test Dependencies: 1. register-admin-standard.py, 2 approve-admin.py

print("Running login-admin-success-change-email.py")
...
try:
    ...
    password_text_box.send_keys('admintest1@gmail.com')
    time.sleep(3)
    ...
    for new_email in new_emails:
        if new_email.text == 'admintest1@gmail.com':
            email_change = True
    ...
    print(ex)
    print("login-admin-success-change-email.py has failed.")
    driver.quit()
    sys.exit(-1)
    
else:
    print("login-admin-success-change-email.py was successful.")
    driver.quit()
