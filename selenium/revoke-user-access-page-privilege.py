...
#README: Login as admin. Then go to admin privileges page. Revoke the deactivate user page privilege (user access page) for admintest1234.
#Then this test will log in as admintest1234 and see if they can navigate to the deactivate user page aka user access page (should fail).

#Test Dependencies: 1. register-admin-standard.py, 2. approve-admin.py

print("Running revoke-user-access-page-privilege.py")
...
try:
    ...
    permit_allow_reject_admin_req_button = driver.find_element_by_xpath('//*[@title="Revoke: deactivateUsers"]')
    if permit_allow_reject_admin_req_button.find_element_by_xpath('./../../td[2]').text == "admintest1234":
        ...
        print("revoked deactivate user page privilege")
        time.sleep(1)
    ...
    page_exists = True
    application_menus = driver.find_elements_by_xpath('//a[@href=\'/deactivateuser\']')
    ...
    print(ex)
    print("revoke-user-access-page-privilege.py has failed.")
    driver.quit()

else:
    print("revoke-user-access-page-privilege.py was successful.")
    driver.quit()
