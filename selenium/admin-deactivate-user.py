import time
from selenium import webdriver
from selenium.webdriver.chrome.options import Options
from selenium.webdriver.support.ui import Select

import sys

#README: Login as admin. Then go to the "Admin deactivate user" page aka the user access page. Then deactivate user test1234.
#Then try to log in as test1234 (should fail)

#Test Dependencies: 1. register-standard.py, 2. login-success-change-password.py

print("Running admin-deactivate-user.py")

try:
        
    if (len(sys.argv) > 1 and sys.argv[1] == "--ci"):
        options = Options()
        options.add_argument('--headless')
        options.add_argument('--no-sandbox')
        options.add_argument('--disable-gpu')  # Last I checked this was necessary.
        driver = webdriver.Chrome(executable_path='/builds/sean/AffordableServerOSU/selenium/linux/chromedriver', chrome_options=options) # List driver in folder
    else:
        driver = webdriver.Chrome('./windows/chromedriver')  # List driver in folder

    driver.get('http://localhost:3000') # Link to affordable application

    time.sleep(3) # Let user see changes

    register_button = driver.find_element_by_xpath('//input[@value=\'Login\']')
    register_button.click()
    # time.sleep(3)


    username_text_box = driver.find_element_by_id('username')
    username_text_box.send_keys('admin')
    # time.sleep(3)

    password_text_box = driver.find_element_by_id('password')
    password_text_box.send_keys('password')
    time.sleep(3)

    login = driver.find_element_by_xpath('//input[@type=\'submit\']')
    login.click()
    time.sleep(3)

    #If the user has an unverified email then hit ok button
    pop_ups = driver.find_elements_by_class_name("swal-title")
    for pop_up in pop_ups:
        if pop_up.text == 'Unverified Email':
            unverified_email_ok_button = driver.find_element_by_class_name("swal-button")
            unverified_email_ok_button.click()
            time.sleep(3)

    #Go to admin registration requests page
    application_menu = driver.find_element_by_xpath('//a[@href=\'/deactivateuser\']')
    application_menu.click()
    time.sleep(3)
    
    #Deactivate test1234, need for loops to go through all users in list
    deactivate_buttons = driver.find_elements_by_class_name("btn-danger")
    for deactivate_button in deactivate_buttons:
        #find deactivate button
        if deactivate_button.text == 'Deactivate':
            #find deactivate button specific to test1234, deactivate test1234 and exit loop
            if deactivate_button.find_element_by_xpath('./../../td[2]').text == "test1234":
                print("deactivated test1234")
                deactivate_button.click()
                break
    
    #wait for deactivate to process
    time.sleep(3)

    #logout, then navigate to login to check if the test1234 can login
    dropdowns = driver.find_elements_by_class_name("dropdown-toggle")
    for dropdown in dropdowns:
        if dropdown.text == 'admin':
            dropdown.click()

    time.sleep(3)
    sign_out_link = driver.find_element_by_link_text("Sign Out")
    sign_out_link.click()

    #login as test1234 to see if deactivation worked
    time.sleep(3)
    username_text_box = driver.find_element_by_id('username')
    username_text_box.send_keys('test1234')
    # time.sleep(3)

    password_text_box = driver.find_element_by_id('password')
    password_text_box.send_keys('Test1234!')
    time.sleep(3)

    login = driver.find_element_by_xpath('//input[@type=\'submit\']')
    login.click()
    time.sleep(3)

    #If the login fails then check reason for failure (user has been deactivated)
    pop_ups = driver.find_elements_by_class_name("swal-text")
    password_failure = False
    for pop_up in pop_ups:
        if pop_up.text == 'Your account is disabled. Please contact an admin.':
            password_failure = True

    assert password_failure == True


except Exception as ex:
    print(ex)
    print("admin-deactivate-user.py has failed.")
    driver.quit()
    sys.exit(-1)

else:
    print("admin-deactivate-user.py was successful.")
    driver.quit()
