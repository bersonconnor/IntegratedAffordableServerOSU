...
#README: Login as admin. Then go to admin privileges page. Give the user access page privilege (deactivate users privilege) to admintest1234.
#Then this test will log in as admintest1234 and see if they can navigate to the user access page (deactivate users page).

#Test Dependencies: 1. register-admin-standard.py, 2. approve-admin.py

print("Running permit-user-access-page-privilege.py")
...
try:
    ...
    permit_allow_reject_admin_req_button = driver.find_element_by_xpath('//*[@title="Permit: deactivateUsers"]')
    if permit_allow_reject_admin_req_button.find_element_by_xpath('./../../td[2]').text == "admintest1234":
        ...
        print("permitted deactivate user page privilege")
        time.sleep(1)
    ...
    assert driver.current_url == 'http://localhost:3000/deactivateuser'

    # STOPS HERE

except Exception as ex:
    print(ex)
    print("permit-user-access-page-privilege.py has failed.")
    driver.quit()

else:
    print("permit-user-access-page-privilege.py was successful.")
    driver.quit()