...
#README: Login as admin. Then go to admin privileges page. Revoke the allow/reject admin requests privilege to admintest1234.
#Then this test will log in as admintest1234 and see if they can navigate to the admin registration request page (should fail).

#Test Dependencies: 1. register-admin-standard.py, 2. approve-admin.py

print("Running revoke-allow-reject-admin-requests-privilege.py")
...
try:
    ...
    permit_allow_reject_admin_req_button = driver.find_element_by_xpath('//*[@title="Revoke: allowRejectAdminRegistration"]')
    if permit_allow_reject_admin_req_button.find_element_by_xpath('./../../td[2]').text == "admintest1234":
        ...
        print("revoked allow/remove admin request privilege")
        time.sleep(1)
    ...
    #check to see if admin requests page exists for admintest1234
    page_exists = True
    application_menus = driver.find_elements_by_xpath('//a[@href=\'/requests\']')
    if len(application_menus) == 0:
        page_exists = False

    assert page_exists == False

    # STOPS HERE

except Exception as ex:
    print(ex)
    print("revoke-allow-reject-admin-requests-privilege.py has failed.")
    driver.quit()

else:
    print("revoke-allow-reject-admin-requests-privilege.py was successful.")
    driver.quit()
