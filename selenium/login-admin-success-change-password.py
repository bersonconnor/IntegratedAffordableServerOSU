...
#README: Navigate to Login page, and login successfully as admintest1234, and then change password on the account. 
# Then login with new password to verify the change worked.

#Test Dependencies: 1. register-admin-standard.py, 2. approve-admin.py
#Dependency note: Running this test before other admin test may break those tests as after this test admintest1234 will have a changed the password
#which breaks every test that requires a login with the new password.

print("Running login-admin-success-change-password.py")
...
try:
    ...
    old_password_text_box.send_keys('admintest1234')
    time.sleep(3)
    ...
    print(ex)
    print("login-admin-success-change-password.py has failed.")
    #driver.quit()
    sys.exit(-1)

else:
    print("login-admin-success-change-password.py was successful.")
    driver.quit()
