...
#README: Login as admin. Then go to admin privileges page. Revoke the privilege to set admin privileges to admintest1234.
#Then this test will log in as admintest1234 and see if they can navigate to the admin privileges page (should fail).

#Test Dependencies: 1. register-admin-standard.py, 2. approve-admin.py

print("Running permit-set-admin-privilege.py")
...
try:
    ...
    #Permit "set admin privileges" privilege for admintest1234
    permit_allow_reject_admin_req_button = driver.find_element_by_xpath('//*[@title="Revoke: setPrivileges"]')
    if permit_allow_reject_admin_req_button.find_element_by_xpath('./../../td[2]').text == "admintest1234":
        ...
        print("revoked set admin privileges privilege")
        time.sleep(1)
    ...
    page_exists = True
    application_menus = driver.find_elements_by_xpath('//a[@href=\'/privileges\']')
    ...
    print(ex)
    print("permit-set-admin-privilege.py has failed.")
    driver.quit()

else:
    print("permit-set-admin-privilege.py was successful.")
    driver.quit()
