...
#README: Navigate to Login page, and fail the login with a wrong password, and then check if the error message says it was due to a password failure.

#Test Dependencies: 1. register-standard.py

print("Running login-fail-incorrect-pass.py")
...
try:
    ...
    print(ex)
    print("login-fail-incorrect-pass.py has failed.")
    driver.quit()
    sys.exit(-1)
    
else:
    print("login-fail-incorrect-pass.py was successful.")
    driver.quit()